...
impl SyncStorage for SqliteSyncStorage {
    ...
    async fn is_object_synced(&self, object_key: &str) -> Result<bool, SyncStorageError> {
        ...
        let result = task::spawn_blocking(move || {
            ...
            let exists: Option<bool> = conn.query_row(
                "SELECT 1 FROM synced_objects WHERE object_key = ?",
                params![object_key],
                |_| Ok(true),
            ).optional().map_err(|e| {
                SyncStorageError::OperationError(format!("Failed to check if object is synced: {}", e))
            })?;
    ...
    async fn update_last_sync_timestamp(&self, timestamp: DateTime<Utc>) 
        ...
        task::spawn_blocking(move || {
            ...
            conn.execute(
                ...
                SyncStorageError::OperationError(format!("Failed to update last sync timestamp: {}", e))
            })?;
    ...
    async fn get_last_sync_timestamp(&self) -> Result<Option<DateTime<Utc>>, SyncStorageError> {
        ...
        let result = task::spawn_blocking(move || {
            ...
            let count: i64 = conn.query_row(
                ...
                SyncStorageError::OperationError(format!("Failed to check sync_state table: {}", e))
            })?;
            ...
            if count == 0 {
                conn.execute(
                    ...
                    SyncStorageError::OperationError(format!("Failed to initialize sync state: {}", e))
                })?;
            ...
            match timestamp_result {
                ...
                Err(e) => {
                    if let rusqlite::Error::InvalidColumnType(..) = e {
                        ...
                        Err(SyncStorageError::OperationError(format!("Failed to get last sync timestamp: {}", e)))
                    }
                }
            }
        }).await.map_err(|e| {
